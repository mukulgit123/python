...
if __name__ == '__main__':
    ...
    if n >= 2 and n <= 10 and sortArr[n-1] <= 100 and sortArr[n-1] >= -100: # Exit when constraints are not met for the
        # highest score
        highestScore = sortArr[n-1]
    else:
        exit()
    for score in sortArr:
        if score >= 100 and score <= -100: # Exit when constraints are not met for remaining scores
            exit()
