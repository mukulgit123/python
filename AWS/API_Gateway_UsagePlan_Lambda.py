...
def apigateway(usage_plans):
    ...
    # print(current_year)
    # print(current_month)
    # List of usage plans to work on
    print("calling from api_gateway function", usage_plans)
    ...
    for usage_id in usage_plans:
        ...
        # Monthly record of quota for each usage plan
        quota = response['items']
        print("Response for usage plan ID {} is {}".format(
            usage_id, quota))  # Response for each API usage plan ID
...
def put_dynamodb(api_key_map):
    ...
    for key, value in api_key_map.items():
        usage_plan_id = key
        # print(key,value)
        usage_name = api_gateway.get_usage_plan(usagePlanId=usage_plan_id)[
            'name']   # Usage Plan name from api gateway
        # print(usage_name)
    ...
    for usage_plan_name, api_key_data in data.items():
        ...
        for key, value in api_key_data.items():
            ...
            for api_key, quota in value.items():
                ...
                if (percentage_consumtion >
                        0):  # Saving records where consumption is greater than 50% into DynamoDB
                    table.put_item(
                        Item={
